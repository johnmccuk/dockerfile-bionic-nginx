...
RUN DEBIAN_FRONTEND=noninteractive apt update -y && DEBIAN_FRONTEND=noninteractive apt install -y vim nginx nodejs npm \
    supervisor php7.3-fpm php7.3-mbstring php7.3-zip php-xdebug php-memcached libssh2-1 libssl1.0.0 php-curl php-ssh2 php7.3-mysql php-dom php-dom composer rsyslog zip unzip && apt clean && service php7.3-fpm start && usermod -u 1000 www-data
